...
import { ArrowLeft, ArrowRight, Check, ChevronLeft, ChevronRight, LoaderCircle, Moon, RotateCcw, Sun, X } from "@lucide/vue";
import { api, type AttemptResult, type AttemptStarted, type AttemptSummary } from "../api";
import { notify } from "../toast";

const props = defineProps<{ quizId: string; darkMode: boolean }>();
...
<template>
  <main class="quiz-page">
    <header class="quiz-top"><button type="button" aria-label="Back to Learning Mode" @click="emit('navigate', attempt ? `/learn/${attempt.quiz.edition_id}` : '/library')"><ArrowLeft :size="19" /></button><strong>Litera Learning</strong><div><span v-if="attempt && !result">{{ answered }} of {{ attempt.quiz.questions.length }} answered</span><button type="button" :aria-label="darkMode ? 'Use light mode' : 'Use dark mode'" :title="darkMode ? 'Light mode' : 'Dark mode'" @click="emit('toggleTheme')"><Sun v-if="darkMode" :size="18" /><Moon v-else :size="18" /></button></div></header>
