      ...
      <section class="availability-section">
        ...
        <div class="availability-list">
          <article v-for="item in digital" :key="item.id">
            <FileText :size="25" /><div><span>Digital / {{ item.file_type }}</span><h3>{{ item.access_level === 'PUBLIC' ? 'Unlimited online access' : item.access_level === 'REGISTERED' ? 'Registered reader access' : 'Member access' }}</h3><p v-if="!item.available">This edition is still being prepared.</p><p v-else-if="hasDigitalAccess(item.access_level)">Open the complete digital edition. Digital books have no copy limit.</p><p v-else>{{ item.access_level === 'REGISTERED' ? 'Sign in to access this digital edition.' : 'This edition is available to active Litera members.' }}</p><button v-if="item.available && hasDigitalAccess(item.access_level)" type="button" @click="emit('navigate', `/read/${item.edition_id}`)">Read now <ArrowRight :size="16" /></button><button v-else-if="item.available" type="button" @click="requestAccess(item.access_level)">{{ item.access_level === 'REGISTERED' ? 'Sign in' : 'View membership' }} <ArrowRight :size="16" /></button><span v-else class="availability-note">{{ formatEnum(item.processing_status) }}</span></div>
          </article>
